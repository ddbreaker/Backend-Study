...
- 변수 캡처
  ...
  - 익명 클래스 또는 로컬 클래스와 다르게 쉐도윙하지 않는다
...
- 인터페이스에 메소드 선언이 아니라 구현체(이미 구현된 메소드)를 제공할 수 있다
  - 따라서 기존 인터페이스를 구현한 클래스를 수정하지 않고 새 기능을 추가할 수 있다
...
- 런타임 에러가 발생할 수 있다는 리스크가 있다.
  ...
  - 또는 기본 메서드를 다시 Override해서 재정의 할 수도 있다

- Object에서 제공하는 메소드는 default method로 재정의할 수 없다(추상 메소드는 가능)

- 인터페이스를 상속받는 인터페이스에서 다시 추상 메소드로 변경할 수 있다

- default 메소드로 인터페이스를 만들어서 상속을 강제하지 않을 수 있다(spring에서 자주 쓰는 방식)
...
- 중개 오퍼레이션
  - Stream을 리턴한다
  - lazy하다(종료 오퍼레이션이 수행되기 전까지 수행되지 않는다)
...
null 체크를 아래와 같이 하면 에러를 발생할 확률이 높다(null 체크를 실수할 가능성이 높다)
...
- Optional filter(Predicate) : true면 그대로 Optional을 return, false면 Optional.empty()을 return

- Optional map(Function) : Optional에 들어있는 값을 변환하고 다시 Optional에 넣어 return
